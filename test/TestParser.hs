...
testFN :: String 
testFN = statementsToString(parseStatements(getTokens(parseTokens(0, "fn add(){}; fn add(a){let five = 5;}; fn add(a,b){let c = a + b; return c;}; fn a(a,b){fn b(a,b){return 5;}; b(1,2);}; fn a(){if(True){let five = 5; return 5;}; return 2;}; fn a(){fn a(){fn a(){fn a(){fn a(a,b){let five = [{1:[{1:[{1:[]}]}]}];};};};};}; fn add((2+3)){};", []))))
...
testIndex :: String 
testIndex = statementsToString(parseStatements(getTokens(parseTokens(0, "let a = b[0]; let a = b[\"c\"]; let a = b[c]; let a = b[1+2]; let a = b[(1+-5)];  let a = b[0]; let a = b[0][0]; let a = b[b[b[b[0]]]]; let a = a[add(2,3)]; let a = b[b[0][0]][0];", []))))

testCall :: String 
testCall = statementsToString(parseStatements(getTokens(parseTokens(0, "add(); add(1); add(1,2); add(a,b); add(\"a\"); add(1+2); add((1+-5)); add(sub()); add(a[0]);", []))))
