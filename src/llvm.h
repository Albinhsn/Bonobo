  ...
  private:
    ...
    llvm::Value *lookupVariable(std::string name) {
        ...
        for (int i = this->function->scopedVariables.size() - 1; i >= 0; i--) {
            std::vector<llvm::AllocaInst *> scope = this->function->scopedVariables[i];
    ...
    bool isString(llvm::Value *value) { return false; }
    ...
    llvm::Value *compileExpression(Expr *expr) {
        ...
        case BINARY_EXPR: {
            ...
            if (llvm::AllocaInst *allocaInst = llvm::dyn_cast<llvm::AllocaInst>(left)) {
                left = this->builder->CreateLoad(allocaInst->getAllocatedType(), allocaInst);
            }
            llvm::Value *right = compileExpression(binaryExpr->right);
            if (llvm::AllocaInst *allocaInst = llvm::dyn_cast<llvm::AllocaInst>(right)) {
                right = this->builder->CreateLoad(allocaInst->getAllocatedType(), allocaInst);
            }
            // String concat
            ...
            for (int i = 0; i < this->structs.size(); ++i) {
                ...
                if (strukt->name == name) {
                    ...
                    if (callExpr->arguments.size() != strukt->fields.size()) {
                        printf("Strukt has different amount of args, expected: %d but got %d",
    ...
    void compileStatement(Stmt *stmt) {
        ...
        case RETURN_STMT: {
            ...
            // Check here if it's an allocaInst and then load it before sending it back
            if (llvm::AllocaInst *allocaInst = llvm::dyn_cast<llvm::AllocaInst>(returnValue)) {
                returnValue = this->builder->CreateLoad(allocaInst->getAllocatedType(), allocaInst);
            }
    ...
    std::map<std::string, llvm::StructType *> setupInternalsStructs() {
        ...
        structs["array"] = llvm::StructType::create(*this->ctx, fieldTypes, "array");
