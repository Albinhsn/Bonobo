...
public:
  ...
  LLVMStruct(std::string name, llvm::StructType *structType,
             std::vector<std::string> fields) {
    this->name = name;
    this->fields = fields;
...
private:
  ...
  llvm::Value *lookupVariable(std::string name) {
    ...
    for (int i = this->function->scopedVariables.size() - 1; i >= 0; i--) {
      std::vector<llvm::AllocaInst *> scope =
          this->function->scopedVariables[i];
      for (int j = 0; j < scope.size(); ++j) {
        if (scope[j]->getName().str() == name) {
          return scope[j];
        }
      }
    }
  ...
  bool checkVariableValueMatch(Variable *var, llvm::Value *&value) {
    if (value->getType() == llvm::Type::getInt32Ty(*this->ctx)) {
      if (var->type == DOUBLE_VAR) {
        value =
            this->builder->CreateUIToFP(value, this->builder->getDoubleTy());
        return true;
      }
  ...
  llvm::Value *castValue(llvm::Value *value) {
    if (llvm::AllocaInst *allocaInst =
      ...
      return this->builder->CreateLoad(allocaInst->getAllocatedType(),
                                       allocaInst);
    }
    return value;
  }
  llvm::Value *compileExpression(Expr *expr) {
    ...
    case BINARY_EXPR: {
      ...
      if (leftType->isIntegerTy() && rightType->isIntegerTy()) {
        ...
        if (leftType->isIntegerTy()) {
          left =
              this->builder->CreateUIToFP(left, this->builder->getDoubleTy());
        } else {
          right =
              this->builder->CreateUIToFP(right, this->builder->getDoubleTy());
        }
    ...
    case DOT_EXPR: {
      ...
      if (llvm::AllocaInst *allocaInst =
              llvm::dyn_cast<llvm::AllocaInst>(value)) {
        value = this->builder->CreateLoad(allocaInst->getAllocatedType(),
                                          allocaInst);
        if (value->getType()->isStructTy()) {
          for (int i = 0; i < this->structs.size(); ++i) {
            ...
            if (strukt->name ==
                allocaInst->getAllocatedType()->getStructName()) {
    ...
    case INDEX_EXPR: {
      ...
      if (variable->getType() == this->internalStructs["string"]) {
        ...
        if (llvm::AllocaInst *allocaInst =
                llvm::dyn_cast<llvm::AllocaInst>(str)) {
    ...
    case ARRAY_EXPR: {
      ...
      std::vector<llvm::Constant *> arrayItems =
          std::vector<llvm::Constant *>(arrayExpr->items.size());
      for (uint64_t i = 0; i < arrayExpr->items.size(); ++i) {
        arrayItems[i] = llvm::dyn_cast<llvm::Constant>(
            compileExpression(arrayExpr->items[i]));
      }
      ...
      llvm::GlobalVariable *globalArray = new llvm::GlobalVariable(
          *this->module, arrayType, false, llvm::GlobalValue::PrivateLinkage,
          arrayConstant);

      llvm::StructType *structType = this->internalStructs["array"];
      llvm::AllocaInst *arrayInstance =
          this->builder->CreateAlloca(structType, nullptr, "array");

      llvm::Value *strGep =
          this->builder->CreateStructGEP(structType, arrayInstance, 0);
      ...
      if (llvm::AllocaInst *allocaInst =
              llvm::dyn_cast<llvm::AllocaInst>(arrayInstance)) {
        return this->builder->CreateLoad(allocaInst->getAllocatedType(),
                                         allocaInst);
      }
    }
    ...
    case CALL_EXPR: {
      ...
      for (int i = 0; i < this->structs.size(); ++i) {
        ...
        // ToDo implement this
        if (strukt->name == name) {
          llvm::AllocaInst *struktInstance =
              builder->CreateAlloca(strukt->structType, nullptr, name);
          ...
          for (int j = 0; j < callExpr->arguments.size(); ++j) {
            llvm::Value *strGep = this->builder->CreateStructGEP(
                strukt->structType, struktInstance, j);
            llvm::Value *paramValue = compileExpression(callExpr->arguments[j]);
            if (strukt->structType->getContainedType(j) !=
                paramValue->getType()) {
      ...
      if (this->libraryFuncs.count(name)) {
        for (int i = 0; i < params.size(); ++i) {
          if (params[i]->getType()->isPointerTy()) {
            while (llvm::AllocaInst *allocaInst =
                       llvm::dyn_cast<llvm::AllocaInst>(params[i])) {
              llvm::Value *structValue = this->builder->CreateLoad(
      ...
      if (((int)func->arg_size()) != params.size()) {
        printf("Calling %s requires %d params but got %d\n", name.c_str(),
               (int)func->arg_size(), (int)params.size());
      }
      int i = 0;
  ...
  void compileStatement(Stmt *stmt) {
    ...
    case RETURN_STMT: {
      ...
      // it back
      returnValue = castValue(returnValue);
    ...
    case STRUCT_STMT: {
      ...
      std::vector<llvm::Type *> fieldTypes =
          std::vector<llvm::Type *>(structStmt->fields.size());
      std::vector<std::string> fieldNames =
          std::vector<std::string>(structStmt->fields.size());
      ...
      llvm::StructType *structType = llvm::StructType::create(
          *this->ctx, fieldTypes, structStmt->name.lexeme);
      this->structs.push_back(
          new LLVMStruct(structStmt->name.lexeme, structType, fieldNames));
      break;
    }
    case IF_STMT: {
      ...
      llvm::BasicBlock *thenBlock =
          llvm::BasicBlock::Create(*ctx, "then", this->function->function);
      llvm::BasicBlock *elseBlock =
          llvm::BasicBlock::Create(*ctx, "else", this->function->function);
      llvm::BasicBlock *mergeBlock =
          llvm::BasicBlock::Create(*ctx, "merge", this->function->function);
    ...
    case FUNC_STMT: {
      ...
      this->function =
          new LLVMFunction(this->function, funcType, funcStmt->name.lexeme,
  ...
  std::map<std::string, llvm::StructType *> setupInternalsStructs() {
    ...
    std::vector<llvm::Type *> fieldTypes = {this->builder->getPtrTy(),
                                            this->builder->getInt32Ty()};
    structs["string"] =
        llvm::StructType::create(*this->ctx, fieldTypes, "string");

    fieldTypes = {this->builder->getPtrTy(), this->builder->getInt32Ty()};
    structs["array"] =
        llvm::StructType::create(*this->ctx, fieldTypes, "array");
...
public:
  LLVMCompiler(std::vector<Stmt *> stmts) {
    ...
    this->function = new LLVMFunction(nullptr, funcType, "main", {}, this->ctx,
                                      this->module);

    this->structs = std::vector<LLVMStruct *>();
    this->libraryFuncs = addLibraryFuncs();
    this->builder = new llvm::IRBuilder<>(this->function->entryBlock);
    this->internalStructs = setupInternalsStructs();
  }
  void compile() {
